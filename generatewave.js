...
var margintop;
var gettingall = document.getElementsByClassName("canvas12");
...
const buttonWidth8 = 60;
const buttonHeight8 = 20;
const buttonX8 = 315
const buttonY8 = 10
ctx.fillStyle = 'blue';
ctx.fillRect(buttonX8, buttonY8, buttonWidth8, buttonHeight8);
...
if (canvas.undo.length ===3){
 canvas.undo.splice(0, 1);
}
canvas.undo.push(canvasarray);
...
canvas.undo.push([...canvas.myNewData.data]);
...
else if (canvas.myNewData.valueindexstart === 0 && canvas.myNewData.data.length>0){
 if (canvas.undo.length ===3){
 canvas.undo.splice(0, 1);
 }
 canvas.undo.push([...canvas.myNewData.data]);
...
else if (x > buttonX8 && x < buttonX8 + buttonWidth8 && y > buttonY8 && y < buttonY8 + buttonHeight8){

if (canvas.undo.length >0){
if (waveformsource){
waveformsource.stop()

}
canvas.myNewData.data= canvas.undo[canvas.undo.length-1];
canvas.undo.pop();
